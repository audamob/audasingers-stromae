...
public class ServiceNewsUpdater extends Service implements IServiceDataUpdater<News> {
		Service instance;
	     ...
	     ThreadLoad(Handler h) {
	      ...
	      public void run() {
	    	  ...
	    	  try {
	  	    		 ...
	  	    		 for (int compteur=0;(compteur<content.size() && List_News.size()<21);compteur++) {
	  	    			 	 Element link = content.get(compteur);
	  	    			 	 Elements element = link.getElementsByClass("title");
	  						 title=element.text();
	  						 Elements e=((Jsoup.parse(element.toString())).getElementsByTag("a"));
	  						 ...
	  						 date=element.outerHtml().substring(element.outerHtml().indexOf("<br />")+6,element.outerHtml().indexOf("</div>"));
	  						
	  						 writer.replace(date,"");
	  						 if(writer.contains("MTV News")){
	  							 writer="MTV News";
	  							 fetch_MTV_News(title,date,description,url,writer,imageUrl);
	  							
	  						 }
	  						 if(writer.contains("MTV Buzzworthy")){
	  							 writer="MTV Buzzworthy";
	  							fetch_MTV_Buzzworthy(title,date,description,url,writer,imageUrl);
	  						 }
	  						 if(writer.contains("VH1 Tuner")){
	  							 writer="VH1 Tuner";
	  							fetch_VH1_Tuner(title,date,description,url,writer,imageUrl);
	  						 }
				 ...
				 String UrlLyrics="http://www.mtv.com/artists/lil-wayne/mtv-news/";
	    		  ...
	    		  for (int compteur=0;(compteur<content.size()&&wasil);compteur++) {
						 ...
						 for(int ttt=0;ttt<List_News.size();ttt++){
							 if(List_News.get(ttt).getUrl().equalsIgnoreCase(url)){
								 wasil=false;
							 }
	      ...
	      private void fetch_MTV_News(String title, String date,String description, String url, String writer, String imageUrl) {
				// TODO Auto-generated method stub
				 Elements content=null;
				 try {
		    	     ...
		    	     News temp=new News(title, date, description, url, writer, imageUrl) ;
	  	...
	  	private void fetch_MTV_Buzzworthy(String title, String date,String description, String url, String writer, String imageUrl) {
			// TODO Auto-generated method stub
			 Elements content=null;
			 try {
				 org.jsoup.nodes.Document doc = Jsoup.connect(url).get();
	    		 ...
	    		 description=content.text();
	    		 content = Jsoup.parse(content.toString()).getElementsByTag("img");
	    		 imageUrl=content.attr("src");
		...
		private void fetch_VH1_Tuner(String title, String date,String description, String url, String writer, String imageUrl) {
			// TODO Auto-generated method stub
			 Elements content=null;
			 try {
				 org.jsoup.nodes.Document doc = Jsoup.connect(url).get();
		 ...
		 public ArrayList<News> restore_News() throws IOException, ClassNotFoundException {
				FileInputStream fin = new FileInputStream(getCacheDir().getAbsolutePath()+"/newsnews");
		 ...
		 public void sauvegard_News(ArrayList<News> s) throws IOException {
				FileOutputStream fout = new FileOutputStream(getCacheDir().getAbsolutePath()+"/newsnews");
		 ...
		 public void News_Notification(String writer, String date, String content,int id){
		    	...
		    	CharSequence contentTitle = "Lil Wayne";
		    	...
		    	RemoteViews contentView = new RemoteViews(getPackageName(), R.layout.audamob_version_deux_notification_layout);
		    	if(id==0){
		    		contentView.setImageViewResource(R.id.imagewriter, R.drawable.audamob_version_deux_mtvnews_c);
		    	}else if(id==1){
		    		contentView.setImageViewResource(R.id.imagewriter, R.drawable.audamob_version_deux_mtvbuzzworthy_c);
		    	}else{
		    		contentView.setImageViewResource(R.id.imagewriter, R.drawable.audamob_version_deux_vh1tuner_c);
		    	}
