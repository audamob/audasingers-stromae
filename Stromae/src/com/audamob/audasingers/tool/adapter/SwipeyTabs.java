...
public class SwipeyTabs extends ViewGroup implements OnPageChangeListener {
	...
	private int mtextColor = 0xff706D6F;
	...
	public SwipeyTabs(Context context) {
		this(context, null);
		// Log.d("Sweipy_tab"," heigh = "+Heigh_Swiper_Tab);
	}
	...
	protected void dispatchDraw(Canvas canvas) {
		if (mCurrentPos != -1) {
			...
			for (int i = 0; i < count; i++) {
				final TextView tab = (TextView) getChildAt(i);
				if (mCurrentPos == i) {
					tab.setTextColor(interpolateColor(mtextColor, mTextColor,
							1.0f - relativePos));
				} else {
	...
	/*public void updateheaderContent(int position) {
		Activity main = MainContainerActivity.activity;
		Log.d("Current_tab", "num = " + position);

		RelativeLayout TabSection1 = (RelativeLayout) main
				.findViewById(R.id.tab_section_1);
		;
		MainContainerActivity.swipe_notdone();
		switch (position) {
		
		case 0:
			if (Build.VERSION.SDK_INT >= 16) {
			TabSection1.setBackground(getResources().getDrawable(
					drawable.tab_settings_bg));
			}else{
				TabSection1.setBackgroundDrawable(getResources().getDrawable(
						drawable.tab_settings_bg));
			}
			MainContainerActivity.request.setVisibility(View.INVISIBLE);
			MainContainerActivity.edit_List.setVisibility(View.INVISIBLE);

			break;
		case 1:
			if (Build.VERSION.SDK_INT >= 16) {
			TabSection1.setBackground(getResources().getDrawable(
					drawable.tab_favorites_bg));
			}else{
				TabSection1.setBackgroundDrawable(getResources().getDrawable(
						drawable.tab_favorites_bg));
			}
			...
			MainContainerActivity.edit_List.setVisibility(View.INVISIBLE);
			try {
				//SwipeyTabFragmentFavorites.update_your_sel();
			} catch (Exception e) {
		...
		case 3:
			...
			MainContainerActivity.edit_List.setVisibility(View.INVISIBLE);
			try {
				SwipeyTabFragmentMusicPlayList.update_your_sel();
			} catch (Exception e) {
				// TODO: handle exception
			}
